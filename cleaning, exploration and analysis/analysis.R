...
# you do not want to run the following lines (93-108).
...
US_ttt <- ggplot(US_time_l, aes(reorder(date2, -c(as.numeric(date))), value, fill = Performance)) +
...
BR_ttt <- ggplot(BR_time_l, aes(reorder(date2, -c(as.numeric(date))), value, fill = Performance)) +
  geom_bar(position="stack", stat="identity") +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=1)) +
  labs(x = "",
       y = "",
...
# Add a dummy in regerssion for elected or not? only on election years.
